...
use Modules\Master\Http\Requests\DivisiRequest;
use Yajra\DataTables\DataTables;

class DivisiController extends Controller
{
    protected $module = 'master.so.divisi';
    protected $route  = 'master.so.divisi';
    protected $perms  = 'master';
    protected $view   = 'master::so.divisi';
        ...
        return DataTables::of($records)
            ...
            ->addColumn(
                ...
                function ($record) use ($user) {
                    // return $record->name;
                    // return $record->name != 'Divisi Human Capital';
                    $actions = [
                        'type:show|id:' . $record->id,
                        'type:edit|id:' . $record->id,
                    ];
                    if ($record->canDeleted() && $record->id !== env('APP_HC_ID')) {
                        // $record->id == env('APP_HC_ID') == Divisi Human Capital
                        $actions[] = [
                            'type'  => 'delete',
                            'id'    => $record->id,
            ...
            ->rawColumns(
                [
                    'code',
                    'name',
                    'parent', 'action', 'status', 'created_at', 'address'
                ]
            )
            ->make(true);
    }
    ...
    {
        $record = OrgStruct::findOrFail($id);
        $parents = OrgStruct::where('id', '!=', $record->id)->bod()->get(['id', 'name']);
