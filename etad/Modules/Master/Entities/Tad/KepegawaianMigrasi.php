...
class KepegawaianMigrasi extends BaseModel
{
    use HasFactory;

    protected $table = 'ref_tad_kepegawaian_migrasi';
    protected $fillable = [
        'status',
    ...
    {
        // dd(77, $value, Carbon::createFromFormat('d/m/Y', $value)->format('Y-m-d'));
    ...
    {
        $this->attributes['out_at'] = $value ? Carbon::createFromFormat('d/m/Y', $value)->format('Y-m-d') : null;
    }
    ...
    public function tad()
    {
        return $this->belongsTo(TadMigrasi::class, 'tad_id');
    }
