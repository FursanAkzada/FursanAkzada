<div class="card card-custom gutter-b">
  ...
  <div class="card-header border-0 pt-5">
    <div class="card-title font-weight-bolder">
      <div class="card-label">Total TAD di {{ auth()->user()->cabang }}
      </div>
    </div>
    <div class="card-toolbar">
      <div class="font-size-sm text-muted mt-2">
        <strong>{{ $vendorsCab->sum('tad_count') }} TAD</strong> dari <strong>{{ $vendorsCab->count() }} Vendor</strong>
      </div>
    </div>
  </div>
  <!--end::Header-->
  ...
  <div class="card-body p-0 d-flex flex-column" style="position: relative;">
    ...
    <div class="flex-grow-1 card-spacer">
      <div class="row">
        @foreach ($vendorsCab->take(9) as $key => $item)
        <!--begin::Item-->
