...
{
    protected $module = 'personil.migrasi';
    protected $route  = 'personil.migrasi';
    protected $view   = 'pengajuan::tad.personil';
    protected $perms  = 'personil.migrasi';
        ...
        $this->setPrepare(
            [
                ...
                'breadcrumb' => [
                    ...
                    'Migrasi Personil'  => route($this->route . '.index'),
                ]
            ]
        );
    }
    ...
    public function ajaxPenilaianTADCekPosisiNoResign(Request $request)
    {
        return Tad::ajaxPenilaianTADCekPosisiNoResign($request);
    }
    ...
    public function grid()
    {
        ...
        $records = TadMigrasi::with('jabatan', 'kepegawaian', 'vendor', 'riwayatKerja', 'lastEmployment')
            ->when(
            ...
            ->when(
                ...
                function ($q) use ($worked) {
                    $q->whereHas(
                        'kepegawaian',
                        function ($w) use ($worked) {
                            $w->whereNull('out_at');
                        }
                    );
                }
            ...
            ->when(
                $not_worked = request()->not_worked,
                function ($q) use ($not_worked) {
                    $q->orWhereDoesntHave('kepegawaian');
                }
            )
            ->when(
                $resign = request()->resign,
                function ($q) use ($resign) {
                    $q->orWhereHas(
                        'kepegawaian',
                        function ($w) use ($resign) {
            ...
            ->editColumn(
                ...
                function ($r) {
                    $buttons[] = [
                        ...
                        'page' => true,
                        'id' => $r->id,
                        'url' => route($this->route . '.show', ['migrasi' => $r->id, 'tab' => 'profile']),
                    ];
                    if (auth()->user()->checkPerms($this->perms . '.edit') && $r->canEdit()) {
                        $buttons[] = [
                            ...
                            'url' => route($this->route . '.keluarga', ['migrasi' => $r->id]),
                        ];
                    ...
                    //     'url' => route($this->route . '.riwayatKerja', ['migrasi' => $r->id]),
                    // ];
                    if (auth()->user()->checkPerms($this->perms . '.delete') && !$r->detail_kandidat && !$r->kepegawaian) {
                        $buttons[] = 'type:delete|id:' . $r->id . '|attrs:data-confirm-text="Hapus ' . $r->nama . '?"';
                    }
    ...
    public function wawancaraHcGrid($tad_id)
    {
        ...
        return DataTables::of($records)
            ...
            ->editColumn(
                'skor',
                function ($r) {
                    return "<span class='badge text-white' style='background-color:brown;'>" . $r->details->sum('value') . "</span>";
                }
            )
            ->editColumn(
                'no_tiket',
    ...
    public function penolakanStore(Kandidat $kandidat)
    {
        return $kandidat->penolakanByRequest();
    }
    public function penerimaanCreate(Kandidat $kandidat)
    {
        $this->setPrepare(
            [
                'perms'  => 'pengajuan.tad.form.approve',
                'title'  => 'Penerimaan ' . $kandidat->tad->nama,
            ]
        );
        return $this->render('pengajuan::tad.form.kandidat.penerimaan', compact('kandidat'));
    }
    public function penerimaanStore(Kandidat $kandidat, Request $request)
    {
        $rules = [
            'accepted'              => ['required', 'in:1,2'],
            'alasan'                => ['required_if:accepted,2'],
            // 'jenis_jabatan' => ['required', 'in:admin,non-admin'],
        ];
        if ($request->accepted == 1) {
            $rules['in_at']         = ['required', 'date_format:d/m/Y'];
            $rules['contract_due']  = ['required', 'date_format:d/m/Y'];
        }
        $request->validate($rules);
        return $kandidat->penerimaanByRequest($request);
    }
    ...
    public function import()
    {
        ...
        return $this->render($this->view . '.import');
    }

    public function template()
    {
        $fileName = 'Template Import Data Personil.xlsx';
        return Excel::download(new PersonilTadTemplateExport, $fileName);
    }
    ...
    public function importSaveConfirm($tipe)
    {
        return $this->render($this->view . '.submit', compact('tipe'));
    }
    public function importSaveConfirmSave(Request $request)
    {
        $record = new Tad;
        return $record->handleImport($request);
    }
