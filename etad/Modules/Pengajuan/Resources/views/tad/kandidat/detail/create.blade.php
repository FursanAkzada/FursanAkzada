<form action="{{ route($route . '.detailStore' , $record->id)}}" autocomplete="off" method="POST">
    @csrf
    ...
    <div class="modal-body">
        <div class="form-group row">
            ...
            <label for="" class="col">: <span class="badge badge-primary">{{ $record->requirement->jabatan->NM_UNIT }}</span></label>
        </div>
        <div class="form-group row">
            <label for="" class="col-4 text-bold">Jumlah Tenaga yg Dibutuhkan</label>
            <label for="" class="col">: <span class="badge" style="background-color: #FF0;">{{ $record->requirement->jumlah }}</span></label>
        </div>
        <div class="form-group row">
            <label for="" class="col-4 text-bold">Kandidat Dari Vendor</label>
            <label for="" class="col">: <span class="badge badge-info">{{ $record->requirement->vendor->nama }}</span></label>
        </div>
