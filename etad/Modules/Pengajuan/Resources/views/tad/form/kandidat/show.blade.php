    ...
    <div class="card-toolbar">
      <div class="dropdown dropdown-inline">
        ...
        <div class="dropdown-menu dropdown-menu-right">
          ...
          <a class="dropdown-item base-modal--render" href="{{ route($route.'.kandidat.wawancara.create',$kandidat->id) }}" 
              data-modal-backdrop="false" 
              data-modal-v-middle="false" 
              data-modal-size="modal-xl"
              data-toggle="tooltip" 
              data-original-title="Form Wawancara" 
              data-placement="bottom"><i class="fas fa-tasks text-primary mr-2"></i>Form Wawancara</a>
              <div class="dropdown-divider"></div>
          @endif
          
          <a class="dropdown-item" href="#"><i class="fab fa-whatsapp text-success mr-2"></i>Undang via Whatsapp</a>
          <a class="dropdown-item" href="#"><i class="fas fa-at text-info mr-2"></i>Undang via Email</a>
          @if (!is_null($kandidat->wawancara))
            @if (is_null($kandidat->accepted))
              <div class="dropdown-divider"></div>
              <a class="dropdown-item base-modal--render" href="{{ route($route.'.kandidat.penerimaan.create',$kandidat->id) }}" 
                    data-modal-backdrop="false" 
                    data-modal-v-middle="false" 
                    data-modal-size="modal-lg"
                    data-toggle="tooltip" 
                    data-original-title="Terima Sebagai Pegawai" 
                    data-placement="bottom"><i class="fas fa-check-circle text-success mr-2"></i>Penerimaan</a>
              <a class="dropdown-item base-modal--confirm" href="{{ route($route.'.kandidat.penolakan.store',$kandidat->id) }}" 
                    data-toggle="tooltip" 
                    data-confirm-text="Yakin <b class='text-danger'>Menolak</b> <b>{{ $kandidat->tad->nama }}</b> Sebagai TAD ?"
                    {{-- data-submit-class="base-form--submit-confirm" --}}
                    data-original-title="Tolak Sebagai Pegawai" 
