@if (isset($tableStruct['datatable_6']))
    <table id="datatable_6" class="table table-bordered table-hover is-datatable hide" style="width: 100%;"
        data-url="{{ isset($tableStruct['table_url_6']) ? $tableStruct['table_url_6'] : route($route . '.grid') }}"
        data-paging="{{ $paging ?? true }}" data-info="{{ $info ?? true }}">
        <thead>
            <tr>
                @foreach ($tableStruct['datatable_6'] as $struct)
