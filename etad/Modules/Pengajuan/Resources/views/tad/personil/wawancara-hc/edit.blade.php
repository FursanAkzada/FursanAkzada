<form action="{{ route($route . '.kandidat.wawancara.update', $wawancara->id) }}" method="POST">
    ...
    <div class="modal-body">
        ...
        @foreach ($kompetensi as $i => $k)
            ...
            <div class="separator separator-dashed separator-border-3 mt-3 mb-5"></div>
            @foreach ($wawancara->penilaian()->whereHas('pertanyaan', function ($q) use ($k) {
            $q->where('kompetensi_id', $k->id);
        })->get() as $i => $p)
                <div class="form-group row">
                    ...
                    <div class="col col-form-label parent-group">
                        <div class="radio-inline">
                            <label class="radio radio-success">
                                <input type="radio" name="question[{{ $p->id }}]"
                                    {{ $p->value == 1 ? 'checked' : '' }} value="1" />
                                <span></span>
                                1
                            </label>
                            <label class="radio radio-success">
                                <input type="radio" name="question[{{ $p->id }}]"
                                    {{ $p->value == 2 ? 'checked' : '' }} value="2" />
                                <span></span>
                                2
                            </label>
                            <label class="radio radio-success">
                                <input type="radio" name="question[{{ $p->id }}]"
                                    {{ $p->value == 3 ? 'checked' : '' }} value="3" />
                                <span></span>
                                3
                            </label>
                            <label class="radio radio-success">
                                <input type="radio" name="question[{{ $p->id }}]"
                                    {{ $p->value == 4 ? 'checked' : '' }} value="4" />
                                <span></span>
                                4
                            </label>
                            <label class="radio radio-success">
                                <input type="radio" name="question[{{ $p->id }}]"
                                    {{ $p->value == 5 ? 'checked' : '' }} value="5" />
                                <span></span>
                                5
                            </label>
                        </div>
            ...
            <div class="row">
                <div class="col-5 parent-group">
                    <select name="kesimpulan" id="" class="form-control base-plugin--select2">
                        <option {{ $wawancara->kesimpulan == 1 ? 'selected' : '' }} value="1">Sangat Disarankan
                        </option>
                        <option {{ $wawancara->kesimpulan == 2 ? 'selected' : '' }} value="2">Disarankan
                        </option>
                        <option {{ $wawancara->kesimpulan == 3 ? 'selected' : '' }} value="3">Cukup Disarankan
                        </option>
                        <option {{ $wawancara->kesimpulan == 4 ? 'selected' : '' }} value="4">Kurang Disarankan
                        </option>
                        <option {{ $wawancara->kesimpulan == 5 ? 'selected' : '' }} value="5">Tidak Disarankan
                        </option>
                        <option {{ $wawancara->kesimpulan == 6 ? 'selected' : '' }} value="6">Tidak Hadir
                        </option>
                    </select>
                </div>
            </div>
