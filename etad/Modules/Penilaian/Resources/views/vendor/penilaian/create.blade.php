...
@section('content')
    <form action="{{ route('penilaian.vendor.store') }}" method="POST">
        @csrf
        <input type="hidden" name="unit_kerja_type" value="{{ $unit_kerja_type }}">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                                ...
                                                @if ($user->cekDivisiHC())
                                                    @foreach ($ORG_STRUCT as $key => $group)
                                                        <optgroup
                                                            ...
                                                            @foreach ($group as $val)
                                                                <option value="{{ $val->id }}"
                                                                    @if ($unit_kerja_id == $val->id) selected @endif>
                                                                    {{ $val->name }}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    @endforeach
                                                @else
                                                    <option value="{{ $user->position->org_struct_id }}">
                                                        {{ $user->position->struct->name }}</option>
                                                @endif
                                            @endif
                                        </select>
                                    </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-8 parent-group">
                                        <select name="vendor_id" id="vendorCtrl"
                                            class="form-control base-plugin--select2-ajax" title="Vendor"
                                            @if ($user->isVendor) data-url="{{ route('master.vendor.ajaxGetByIdUnitKerja') }}"
                                            @else
                                                @if ($user->cekDivisiHC())
                                                    data-url="{{ route('master.vendor.ajaxGetByIdUnitKerja', ['unit_kerja_id' => $unit_kerja_id]) }}"
                                                @else
                                                    data-url="{{ route('master.vendor.ajaxGetByIdUnitKerja', ['unit_kerja_id' => $user->position->org_struct_id]) }}" @endif
                                            @endif
                                            title="Vendor">
                                        </select>
                                    </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-4 parent-group">
                                        <input class="form-control base-plugin--datepicker-3" id="yearCtrl" name="tahun"
                                            data-options="{&quot;startDate&quot;:&quot;+0Y&quot;}" data-orientation="bottom"
                                            placeholder="Tahun">
                                    </div>
                                    <div class="col-md-4 parent-group">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-8 parent-group">
                                        <select class="form-control base-plugin--select2-ajax"
                                            ...
                                            id="toCtrl" name="to" title="Kepada">
                                            <option value=""></option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col col-form-label parent-group"
                                    ...
                                    <div class="radio-inline">
                                        ...
                                        <label class="radio radio-success">
                                            <input type="radio" name="question[{{ $item->id }}]" value="4" />
                                            <span></span>
                                            4
                                        </label>
                                        <label class="radio radio-success">
                                            <input type="radio" checked name="question[{{ $item->id }}]"
                                                value="5" />
                                            <span></span>
                                            5
                                        </label>
...
@push('scripts')
    <script>
        ...
        $('#modal .modal-dialog-right-bottom').removeClass('modal-dialog-right-bottom');
