...
class PullPersonilNonAktif extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'pull:personil-non-aktif';
     ...
     * @var string
     */
    protected $description = 'Ambil personil non aktif dari EHC ke e-TAD';
    ...
    {
        $this->output->title('Synchronizing personil non aktif dari EHC ke e-TAD');
        if (env('DB_SYNC', false)) {
            ...
            $response = Http::withHeaders(
                ...
                ->withoutVerifying()
                ->post(
                    'https://sriwijaya.bankjatim.co.id/EHC_SURROUNDING/rest/ETAD/NonTAD',
                    $payload
                );
            ...
            try {
                foreach (($response_json['ListActive'] ?? []) as $key => $TAD) {
                    ...
                    if(!$failed){
                        ...
                        $kepegawaianMigrasi->status        = KepegawaianMigrasi::RESIGN;
